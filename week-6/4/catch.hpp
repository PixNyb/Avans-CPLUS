...
public:
  ...
  template <typename T> void captureValues(size_t index, T &&value) {
...
template <typename T> class SingleValueGenerator : public IGenerator<T> {
  T m_value;
...
template <typename T> class FixedValuesGenerator : public IGenerator<T> {
...
template <typename T> class RangeGenerator : public IGenerator<T> {
  T const m_first;
  T const m_last;
...
template <typename T> struct NullGenerator : IGenerator<T> {
...
template <typename T> class GeneratorRandomiser : public IGenerator<T> {
...
template <typename T> struct RequiresASpecialisationFor;
...
template <typename T> struct Generators : GeneratorBase {
